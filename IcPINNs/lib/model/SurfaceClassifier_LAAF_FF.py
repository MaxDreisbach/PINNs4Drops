...
from .FourierFeatures import GaussianFourierFeatureTransform


class SurfaceClassifier_LAAF_FF(nn.Module):
    def __init__(self, filter_channels, num_views=1, no_residual=True, last_op=None, LAAF_scale=2.0, num_dims=3, encoding_dim=128, encoding_scale=10):
        super(SurfaceClassifier_LAAF_FF, self).__init__()
        ...
        self.encoding.append(GaussianFourierFeatureTransform(self.num_dims, self.encoding_dim, self.encoding_scale))
        self.add_module("rff_encoding", self.encoding[0])
    ...
    def forward(self, im_feat, x_feat, y_feat, z_feat, t_feat):
        ...
        ''' Get random fourier feature embedding of input coordinates'''
        xyz = torch.cat([x_feat, y_feat, z_feat], 1)
        #print('xyz network input size: ', xyz.size())
        xyz_encoding = self._modules['rff_encoding'](xyz)
        #print('xyz_encoding network input size: ', xyz_encoding.size())

        y = torch.cat([im_feat, xyz_encoding, t_feat], 1)
        tmpy = torch.cat([im_feat, xyz_encoding, t_feat], 1)
        y = y.squeeze(0)
        y = y.swapaxes(0, 1)
        tmpy = tmpy.squeeze(0)
