...
class TrainDataset(Dataset):
    ...
    def select_sampling_method(self, subject):
        ...
        #ToDo: replace fill zero with fill float('nan') -> labels_u = interpn(grid_points, u_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
        samplesT = np.transpose(uvwp_samples, (1, 0))
        labels_u = interpn(grid_points, u_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
        labels_v = interpn(grid_points, v_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
        labels_w = interpn(grid_points, w_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
        labels_p = interpn(grid_points, p_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
        # labels_c = interpn(grid_points, c_grid, samplesT[:, :], bounds_error=False, fill_value=float('nan'))
