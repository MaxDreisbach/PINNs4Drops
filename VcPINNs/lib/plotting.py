...
def plot_contour_w_alpha(opt, samples, preds, alpha, labels, labels_alpha, plane_dim, name, type, sample_name, dataset_type):
    ...
    err_plot[ :int((ground + 1) * opt.resolution / 256), :] = 0
    ...
    if type == 'vel':
        ...
        #levels = np.linspace(-0.75, 0.75, 100)
        #cbar_ticks = [-0.75, -0.35, 0.0, 0.35, 0.75]
        #levels_err = np.linspace(0.0, 0.25, 100)
        #cbar_ticks_err = [0.0, 0.05, 0.1, 0.15, 0.2, 0.25]
        cbar_label = '$%s\,\mathrm{[m/s]}$' %name
        colormap = 'RdBu_r'
    if type == 'pres':
        levels = np.linspace(-150.0, 600, 100)
        cbar_ticks = [-150.0, 0.0, 150, 300, 450, 600]
        levels_err = np.linspace(0.0, 200, 100)
        cbar_ticks_err = [0, 40, 80, 120, 160, 200]
        #levels = np.linspace(-120.0, 480, 100)
        #cbar_ticks = [-120.0, 0.0, 120, 240, 360, 480]
        #levels_err = np.linspace(0.0, 160, 100)
        #cbar_ticks_err = [0, 40, 80, 120, 160]
        cbar_label = '$p\,\mathrm{[Pa]}$'
        colormap = 'viridis'
    ...
    axs[0].set_ylabel(axis_label_y, fontsize=font_size)
    axs[0].set_xlabel(axis_label_x, fontsize=font_size)
    axs[1].set_xlabel(axis_label_x, fontsize=font_size)
    axs[2].set_xlabel(axis_label_x, fontsize=font_size)
    ...
    axs[0].set_title('$%s_{\mathrm{pred}}$' %name, fontsize=font_size, y=1, pad=20)
...
def plot_contour_w_alpha_res_gt(opt, samples, preds, alpha, labels_alpha, labels, plane_dim, name, type, sample_name, dataset_type):
    ...
    axs[0].set_title('$%s_{\mathrm{pred}}$' %name, fontsize=20, y=1, pad=20)
...
def plot_compound(opt, samples, res_PINN, labels_alpha, labels_u, labels_v, labels_w, labels_p, plane_dim, name, type, sample_name, dataset_type):
    ...
    cbar1.set_ticks(cbar_ticks)
    cbar2.set_ticks(cbar_ticks)
    cbar1.set_label('$p\,\mathrm{[Pa]}$', fontsize=font_size, labelpad=0.1)
    cbar2.set_label('$p\,\mathrm{[Pa]}$', fontsize=font_size, labelpad=0.1)
...
def plot_compound_crop(opt, samples, res_PINN, labels_alpha, labels_u, labels_v, labels_w, labels_p, plane_dim, name, type, sample_name, dataset_type, crop_ratio=(0.5, 0.5), crop_offset = (0.0, 0.1), plot_streamlines=True):
    """
    Plot predicted and ground truth fields on a specified 2D plane.
    Allows cropping of the plotted region via crop_ratio=(fx, fy).

    Parameters
    ----------
    opt : object
        Contains settings such as opt.resolution and opt.name.
    samples : torch.Tensor
        Sampled 3D coordinates.
    res_PINN : torch.Tensor
        PINN-predicted quantities (alpha, u, v, w, p).
    labels_* : torch.Tensor
        Ground truth field labels.
    plane_dim : str
        'x', 'y', or 'z' – which plane to visualize.
    name : str
        Field name or descriptive tag.
    type : str
    ...
    crop_ratio : tuple(float, float)
        (fraction_x, fraction_y) of the plane to plot.
        e.g., (0.5, 0.5) → lower-left quarter of the domain.
    """
    font_size = 24.0

    # Convert to NumPy
    sample = samples.detach().cpu().numpy().T
    label_alpha = labels_alpha.detach().cpu().numpy()
    label_u = labels_u.detach().cpu().numpy()
    label_v = labels_v.detach().cpu().numpy()
    label_w = labels_w.detach().cpu().numpy()
    ...
    ground = 28
    ground = 0
    solid_mask = slice(0, int(28 * opt.resolution / 256))
    alpha_interpn[:, solid_mask, :] = 0
    u_interpn[:, solid_mask, :] = 0
    v_interpn[:, solid_mask, :] = 0
    w_interpn[:, solid_mask, :] = 0
    p_interpn[:, solid_mask, :] = 0
    label_alpha_interpn[:, solid_mask, :] = 0
    label_u_interpn[:, solid_mask, :] = 0
    label_v_interpn[:, solid_mask, :] = 0
    label_w_interpn[:, solid_mask, :] = 0
    label_p_interpn[:, solid_mask, :] = 0

    # extract 2D plane slices
    if plane_dim == 'x':
        alpha_plot = np.squeeze(alpha_interpn[0, :, :])
        u_plot = np.squeeze(u_interpn[0, :, :])
        v_plot = np.squeeze(v_interpn[0, :, :])
    ...
    # --- CROP REGION WITH TRANSLATION ---
    crop_ratio_x, crop_ratio_y = crop_ratio
    crop_offset_x, crop_offset_y = crop_offset
    
    nx = int(opt.resolution * crop_ratio_x)
    ny = int(opt.resolution * crop_ratio_y)
    ox = int(opt.resolution * crop_offset_x)
    oy = int(opt.resolution * crop_offset_y)
    
    # Ensure bounds don’t exceed domain
    x_start, x_end = ox, min(ox + nx, opt.resolution)
    y_start, y_end = oy, min(oy + ny, opt.resolution)
    
    x = x[y_start:y_end, x_start:x_end]
    y = y[y_start:y_end, x_start:x_end]
    alpha_plot = alpha_plot[y_start:y_end, x_start:x_end]
    label_alpha_plot = label_alpha_plot[y_start:y_end, x_start:x_end]
    p_plot = p_plot[y_start:y_end, x_start:x_end]
    label_p_plot = label_p_plot[y_start:y_end, x_start:x_end]
    
    if plane_dim == 'x':
        w_plot, v_plot = w_plot[y_start:y_end, x_start:x_end], v_plot[y_start:y_end, x_start:x_end]
        label_w_plot, label_v_plot = label_w_plot[y_start:y_end, x_start:x_end], label_v_plot[y_start:y_end, x_start:x_end]
    elif plane_dim == 'y':
        u_plot, w_plot = u_plot[y_start:y_end, x_start:x_end], w_plot[y_start:y_end, x_start:x_end]
        label_u_plot, label_w_plot = label_u_plot[y_start:y_end, x_start:x_end], label_w_plot[y_start:y_end, x_start:x_end]
    elif plane_dim == 'z':
        u_plot, v_plot = u_plot[y_start:y_end, x_start:x_end], v_plot[y_start:y_end, x_start:x_end]
        label_u_plot, label_v_plot = label_u_plot[y_start:y_end, x_start:x_end], label_v_plot[y_start:y_end, x_start:x_end]
    ...
    fig, axs = plt.subplots(nrows=1, ncols=2, sharex=False, sharey=True, figsize=(10, 6.85)) #6.85
    p1 = axs[0].contourf(x, y, p_plot, levels=levels, cmap=colormap)
    p2 = axs[1].contourf(x, y, label_p_plot, levels=levels, cmap=colormap)

    for c in p1.collections + p2.collections:
        c.set_edgecolor("face")

    levels_alpha = np.linspace(0.5, 1.0, 2)
    axs[0].contour(x, y, alpha_plot, levels=levels_alpha, colors='k')
    axs[1].contour(x, y, label_alpha_plot, levels=levels_alpha, colors='k')
    
    if plot_streamlines:
        # --- STREAMLINES instead of quivers ---
        lw = 1.5              # streamline width
        streamline_density = 1.2  # how many streamlines to draw
        
        if plane_dim == 'x':
            axs[0].streamplot(x, y, w_plot, v_plot, color='white', density=streamline_density, linewidth=lw)
            axs[1].streamplot(x, y, label_w_plot, label_v_plot, color='white', density=streamline_density, linewidth=lw)
        if plane_dim == 'y':
            axs[0].streamplot(x, y, u_plot, w_plot, color='white', density=streamline_density, linewidth=lw)
            axs[1].streamplot(x, y, label_u_plot, label_w_plot, color='white', density=streamline_density, linewidth=lw)
        if plane_dim == 'z':
            axs[0].streamplot(x, y, u_plot, v_plot, color='white', density=streamline_density, linewidth=lw)
            axs[1].streamplot(x, y, label_u_plot, label_v_plot, color='white', density=streamline_density, linewidth=lw)
    else:
        skip = (slice(None, None, 10), slice(None, None, 10))
        arr_color = 'white'
        arr_scale = 7.5 * crop_ratio_x
        
        if plane_dim == 'x':
            axs[0].quiver(x[skip], y[skip], w_plot[skip], v_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)
            axs[1].quiver(x[skip], y[skip], label_w_plot[skip], label_v_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)
        elif plane_dim == 'y':
            axs[0].quiver(x[skip], y[skip], u_plot[skip], w_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)
            axs[1].quiver(x[skip], y[skip], label_u_plot[skip], label_w_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)
        elif plane_dim == 'z':
            axs[0].quiver(x[skip], y[skip], u_plot[skip], v_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)
            axs[1].quiver(x[skip], y[skip], label_u_plot[skip], label_v_plot[skip], scale=arr_scale, scale_units='inches', color=arr_color)

    axs[0].set_ylabel(axis_label_y, fontsize=font_size)
    axs[0].set_xlabel(axis_label_x, fontsize=font_size)
    axs[1].set_xlabel(axis_label_x, fontsize=font_size)

    xt = np.arange(crop_offset_x, crop_ratio_x + crop_offset_x + 0.001, 0.1)
    yt = np.arange(crop_offset_y, crop_ratio_y + crop_offset_y + 0.001, 0.1)
    axs[0].set_xticks(xt)
    axs[0].set_yticks(yt)
    axs[1].set_xticks(xt)
    axs[1].set_yticks(yt)
    ...
    axs[0].tick_params(axis='both', which='major', labelsize=font_size, pad=10)
    axs[1].tick_params(axis='both', which='major', labelsize=font_size, pad=10)
    axs[0].set_title(r'$\mathrm{prediction}$', fontsize=font_size, y=1, pad=20)
    axs[1].set_title(r'$\mathrm{ground\ truth}$', fontsize=font_size, y=1, pad=20)
    ...
    # formating
    for ax in axs:
        ax.set_xlim(crop_offset_x, crop_ratio_x + crop_offset_x)
        ax.set_ylim(crop_offset_y, crop_ratio_y + crop_offset_y)
        ax.margins(x=0, y=0)  # remove axis padding
        ax.set_aspect('equal', adjustable='box')  # preserve aspect ratio

    filename = f'results/{opt.name}/pred_fields/{dataset_type}_{sample_name}_{name}_{plane_dim}_pred.pdf'
    plt.savefig(filename)
    plt.close(fig)
...
def plot_contour_grid(pred, label, plane_dim, res, type):
    ...
    axs[0].set_title(r'$U_{\mathrm{pred}}$', fontsize=16, y=1, pad=20 )
...
def plot_iso_surface_eval(opt, samples, preds, name, sample_name, dataset_type):
    ...
    camera.position = (1.0, 0.075, 1.5)
    camera.focal_point = (0.0, -0.5, 0.5)
    ...
    # Apply the camera to the plotter
    p.camera = camera  # This ensures the plotter knows about the updated camera settings
    p.render()         # Force render to apply the camera changes
    ...
    if OFF_SCREEN:
        ...
        filename = 'results/' + dataset_type + '_' + sample_name + '_'  + '_pred_3d.svg'
        p.save_graphic(filename)
        p.show(auto_close=False)   # open interactive viewer
        print(p.camera_position)   # after rotating manually, print camera settings
...
def plot_contour_eval(coords, opt, preds, alpha, plane_dim, name, y_ground, type, sample_name):
    font_size = 18.0
    # interpolate point cloud to 2D-plane
    ...
    # mask out prediction in solid domain
    ground = y_ground
    ground_index = int(ground * opt.resolution / 2)
    #print(ground, ground_index)
    ...
    if type == 'alpha':
        ...
        cbar_label = '$%s\,\mathrm{[-]}$' %name
        colormap = 'RdBu_r'
    elif type == 'vel_PDMS':
        vmin, vmax = -0.8, 0.8
        levels = np.linspace(-0.8, 0.8, 100)
        cbar_ticks = [-0.7, -0.35, 0, 0.35, 0.7]
        cbar_label = '$%s\,\mathrm{[m/s]}$' %name
        colormap = 'RdBu_r'
    elif type == 'vel_FDM':
        vmin, vmax = -0.45, 0.45
        levels = np.linspace(-0.45, 0.45, 100)
        cbar_ticks = [-0.4, -0.2, 0, 0.2, 0.4]
        cbar_label = '$%s\,\mathrm{[m/s]}$' %name
        colormap = 'RdBu_r'
    elif type == 'pres_PDMS':
        vmin, vmax = -250, 1000
        levels = np.linspace(-250, 1000, 100)
        cbar_ticks = [-250.0, 0.0, 250, 500, 750, 1000]
        cbar_label = '$%s\,\mathrm{[Pa]}$' %name
        colormap = 'viridis'
    elif type == 'pres_FDM':
        vmin, vmax = -80, 320
        levels = np.linspace(-80, 320, 100)
        cbar_ticks = [-80.0, 0.0, 80, 160, 240, 320]
        cbar_label = '$%s\,\mathrm{[Pa]}$' %name
        colormap = 'viridis'
    else:
    ...
    axs.set_ylabel(axis_label_y, fontsize=font_size)
    axs.set_xlabel(axis_label_x, fontsize=font_size)
    ...
    axs.set_title('$%s_{\mathrm{pred}}$' % name, fontsize=font_size, pad=20)
...
def plot_contour(opt, samples, preds, labels, plane_dim, name, type, sample_name, dataset_type):
    ...
    axs[0].set_title(r'$\alpha_{\mathrm{pred}}$', fontsize=40, y=1, pad=20)
