...
class HGPIFuNet(BasePIFuNet):
    ...
    def __init__(self,
        ...
        if not opt.use_LAAF:
            ...
            self.surface_classifier = SurfaceClassifier_LAAF(
                filter_channels=self.opt.mlp_dim,
                temp_filters=self.opt.tcn_dim,
                num_views=self.opt.num_views,
                no_residual=self.opt.no_residual,
                residual_tcn=self.opt.residual_tcn,
                last_op=nn.Sigmoid(),
    ...
    def get_dimensional_pred(self, set_eval=True):
        # retrieve de-normalized data for u,v,w,p
        alpha = self.preds[:, 0, :]
        u = de_norm(self.pred[:, 1, :], self.umin, self.umax)
        v = de_norm(self.pred[:, 2, :], self.vmin, self.vmax)
        ...
        if set_eval:
            ...
            with open(os.path.join(self.opt.test_folder_path, "exp_case.json"), "r") as f:
                exp_case = json.load(f)
        
            self.U_ref = exp_case["U_0"]  # impact velocity
            #self.L_ref = exp_case["rp"]  # image reproduction scale -> domain size
    ...
    def query(self, points, calibs, transforms=None, labels=None, uvwp_points=None, residual_points=None, labels_u=None, labels_v=None, labels_w=None,
        ...
        if labels is not None:
            self.labels = labels
            # p_a = points[1:2, :, :self.n_data]
            # plot_data_sample(p_a[:, :1, :], p_a[:, 1:2, :], p_a[:, 2:3, :], labels, 0.0, 1.0)

        if labels_u is not None and labels_v is not None and labels_w is not None:
            labels_u_proj, labels_w_proj = project_velocity_vector_field(labels_u, labels_w, calibs[1:2])
            # p_plot = points[1:2, :, self.n_data:self.n_data * 2]
            # plot_data_sample(p_plot[:, :1, :], p_plot[:, 1:2, :], p_plot[:, 2:3, :], labels_u, -2.0, 2.0)
            #plot_data_sample(p_plot[:, :1, :], p_plot[:, 1:2, :], p_plot[:, 2:3, :], labels_u_proj, -2.0, 2.0)
        ...
        #x = xyz[:, 0:1, :]
        #z = xyz[:, 2:3, :]
        #print('x mean: ', x.mean().item(), 'max: ', x.max().item(), 'min: ', x.min().item())
        #print('z mean: ', z.mean().item(), 'max: ', z.max().item(), 'min: ', z.min().item())
        xy = xyz[:, :2, :]
        in_img = (xy[:, 0] >= -1.0) & (xy[:, 0] <= 1.0) & (xy[:, 1] >= -1.0) & (xy[:, 1] <= 1.0)

        '''non-dimensionalize coordinates
        Flip y-axis - required as it was flipped in TrainDataset'''
        self.x_feat = xyz[self.num_views // 2: self.num_views // 2 + 1, :1, :]
        self.y_feat = -xyz[self.num_views // 2: self.num_views // 2 + 1, 1:2, :]
        self.z_feat = xyz[self.num_views // 2: self.num_views // 2 + 1, 2:3, :]
        ...
        for im_feat in self.im_feat_list:
            ...
            ''' Masking output sampled points outside of image domain (with middle image of stack) '''
            pred_occupancy = in_img[1:2, None].float() * self.pred[:, :1, :]
    ...
    def get_velocity_loss(self, points):
        ...
        if points is not None:
            ...
            solid_mask  = self.get_solid_domain_mask(points)
            
            pred_u = pred_u * solid_mask
            pred_v = pred_v * solid_mask
            pred_w = pred_w * solid_mask
            self.labels_u = self.labels_u * solid_mask
            self.labels_v = self.labels_v * solid_mask
            self.labels_w = self.labels_w * solid_mask
            
            # Mask out NaNs (i.e., out-of-bounds sampling points)
            valid_mask_u = ~torch.isnan(self.labels_u)
            valid_mask_v = ~torch.isnan(self.labels_v)
            valid_mask_w = ~torch.isnan(self.labels_w)
            
            error_u = self.error_term(pred_u[valid_mask_u], self.labels_u[valid_mask_u])
            error_v = self.error_term(pred_v[valid_mask_v], self.labels_v[valid_mask_v])
            error_w = self.error_term(pred_w[valid_mask_w], self.labels_w[valid_mask_w])   
        else:
            error_u = 0
            error_v = 0
            error_w = 0
    ...
    def get_pressure_loss(self, points):
        ...
        if points is not None:
            ...
            pred_p = pred_p * solid_mask
            self.labels_p = self.labels_p * solid_mask
            
            # Mask out NaNs (i.e., out-of-bounds)           
            valid_mask_p = ~torch.isnan(self.labels_p)
            
            error_p = self.error_term(pred_p[valid_mask_p], self.labels_p[valid_mask_p])
        else:
            error_p = 0
    ...
    def get_pde_loss(self, points):
        ...
        #t_c = rho_M / self.rho_ref * (u * w_x + v * w_y + w * w_z)
        #t_p = p_z
        #t_d = - one_Re * (w_xx + w_yy + w_zz) - 2 * one_Re_z * w_z - one_Re_y * (v_z + w_y) - one_Re_x * (u_z + w_x)
        #t_s = - f_sigma_z
        #t_g = rho_M / self.rho_ref * one_Fr2
        
        #print("||w_z||", torch.mean(torch.abs(w_z)))
        #print("||w_zz||", torch.mean(torch.abs(w_zz)))
        #print("||f_sigma_z||", torch.mean(torch.abs(f_sigma_z)))
        #print("||p_x||", torch.mean(torch.abs(p_x)))
        #print("||p_y||", torch.mean(torch.abs(p_y)))
        #print("||p_z||", torch.mean(torch.abs(p_z)))
        
        #print("Mean |p_z|:", torch.mean(torch.abs(p_z)).item())
        #print("Max |p_z|:", torch.max(torch.abs(p_z)).item())
        #print("Std  p_z:", torch.std(p_z).item())
        #print("p range:", p.min().item(), p.max().item())
        
        #print("||temp||", torch.mean(torch.abs(t_t)))
        #print("||conv||", torch.mean(torch.abs(t_c)))
        #print("||pres||", torch.mean(torch.abs(t_p)))
        #print("||diff||", torch.mean(torch.abs(t_d)))
        #print("||surf||", torch.mean(torch.abs(t_s)))
        #print("||momZ||", torch.mean(torch.abs(res_momentum_z)))

        #print('temporal term - min:', torch.min(t_t).item(), ' max: ', torch.max(t_t).item(), ' mean abs: ',torch.mean(torch.abs(t_t)).item())
        #print('convective term - min: ', torch.min(t_c).item(), ' max: ', torch.max(t_c).item(), ' mean abs: ',torch.mean(torch.abs(t_c)).item())
        #print('pressure term - min: ', torch.min(t_p).item(), ' max: ', torch.max(t_p).item(), ' mean abs: ',torch.mean(torch.abs(t_p)).item())
        #print('diffusive term - min: ',  torch.min(t_d).item(), ' max: ', torch.max(t_d).item(), ' mean abs: ',torch.mean(torch.abs(t_d)).item())
        #print('surface tension - min: ', torch.min(t_s).item(), ' max: ', torch.max(t_s).item(), ' mean abs: ',torch.mean(torch.abs(t_s)).item())
        #print('gravity - min: ', torch.min(t_g).item(), ' max: ', torch.max(t_g).item(), ' mean abs: ',torch.mean(torch.abs(t_g)).item())
        #print('residual mom z - min: ', torch.min(res_momentum_y).item(), ' max: ', torch.max(res_momentum_y).item(), ' mean abs: ',torch.mean(torch.abs(res_momentum_y)).item())
        ...
        #print("||alpha_t||", torch.mean(torch.abs(alpha_t)))
        #print("||alpha_x||", torch.mean(torch.abs(alpha_x)))
        #print("||alpha_y||", torch.mean(torch.abs(alpha_y)))
        #print("||alpha_z||", torch.mean(torch.abs(alpha_z)))
        
        #print("||u * alpha_x||", torch.mean(torch.abs(u * alpha_x)))
        #print("||v * alpha_y||", torch.mean(torch.abs(v * alpha_y)))
        #print("||w * alpha_z||", torch.mean(torch.abs(w * alpha_z)))
        
        #print("||u_x||", torch.mean(torch.abs(u_x)))
        #print("||v_y||", torch.mean(torch.abs(v_y)))
        #print("||w_z||", torch.mean(torch.abs(w_z)))
        ...
        #print('res_conti shape: ', res_conti.shape)
        #plot_data_sample(self.x[0, 0, n_start:], self.y[0, 0, n_start:], self.z[0, 0, n_start:], res_conti, torch.min(res_conti).item(), torch.max(res_conti).item())
