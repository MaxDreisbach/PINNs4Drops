...
class HGPIFuNet_CH2(BasePIFuNet):
    ...
    def __init__(self,
        ...
        if not opt.use_LAAF:
            ...
            self.surface_classifier = SurfaceClassifier_CH2(
    ...
    def get_velocity_loss(self, points):
        ...
        if points is not None:
            ...
            # Combine masks
            mask_u = solid_mask * valid_mask_u
            mask_v = solid_mask * valid_mask_v
            mask_w = solid_mask * valid_mask_w
            
            error_u = self.error_term(pred_u * mask_u, self.labels_u * mask_u)
            error_v = self.error_term(pred_v * mask_v, self.labels_v * mask_v)
            error_w = self.error_term(pred_w * mask_w, self.labels_w * mask_w)
        else:
            error_u = 0
            error_v = 0
            error_w = 0
    ...
    def get_pressure_loss(self, points):
        ...
        if points is not None:
            ...
            # Mask out NaNs (i.e., out-of-bounds)
            valid_mask_p = ~torch.isnan(self.labels_u)
            mask_p = solid_mask * valid_mask_p
            
            error_p = self.error_term(pred_p * mask_p, self.labels_p * mask_p)
        else:
            error_p = 0
