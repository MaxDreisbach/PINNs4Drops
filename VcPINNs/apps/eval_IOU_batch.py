...
for i,epoch in enumerate(epochs):
  ...
  if args.LAAF:
      command = base_command + ' --gpu_id ' + str(args.GPU_ID) + ' --batch_size ' + str(args.num_files) +' --dataroot ' + args.data_folder + ' --name ' + args.file_dir + ' --load_netG_checkpoint_path ./checkpoints/' + args.file_dir + '/netG_epoch_' + str(i) + ' --resume_epoch ' + str(i) + ' --n_data 15000 ' + '--num_views 5'
  else:
      command = base_command + ' --gpu_id ' + str(args.GPU_ID) + ' --batch_size ' + str(args.num_files) +' --dataroot ' + args.data_folder + ' --name ' + args.file_dir + ' --load_netG_checkpoint_path ./checkpoints/' + args.file_dir + '/netG_epoch_' + str(i) + ' --resume_epoch ' + str(i)  + ' --n_data 15000 ' + '--num_views 5'
