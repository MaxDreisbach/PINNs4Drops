    ...
    with open(opt_log, 'w') as outfile:
        ...
        with torch.no_grad():
            ...
            if not opt.no_num_eval:
                ...
                test_errors = calc_error(opt, netG, cuda, test_dataset,  296, ds='eval_complete', plot_results=False, eval_droplet_only=False)
                ...
                #train_errors = calc_error(opt, netG, cuda, train_dataset, int(len(test_data_loader)/36), ds='eval_complete', plot_results=False)
            ...
            if not opt.no_gen_mesh:
                ...
                for gen_idx in tqdm(range(len(test_data_loader)//36)):
                    save_path = '%s/%s/test_eval_epoch%d_%s.obj' % (
                        opt.results_path, opt.name, 0, test_dataset[gen_idx]['name'])
                    print(save_path)
                    gen_mesh(opt, netG, cuda, test_dataset[gen_idx], save_path, use_octree=False, gen_vel_pres=False, gen_3D_iso=False, process_val_data=True)
