...
if __name__ == '__main__':
    ...
    skip_len = opt.num_views // 2
    for id in range(len(test_images)):
        # spare out first and last images for temporal processing depending on temporal window size
        if id <= skip_len - 1 or id >= len(test_images) - skip_len:
