...
class EvalDataset2(Dataset):
    ...
    def __init__(self, opt, phase='test', slice_dim='z'):
        ...
        self.num_sample_inout = self.opt.n_data
    ...
    def select_sampling_method(self, subject):
        ...
        surface_points, _ = trimesh.sample.sample_surface(mesh, self.n_data*1000)
        sample_points = surface_points + np.random.normal(scale=self.opt.sigma, size=surface_points.shape)

        # add random points within image space
        length = self.B_MAX - self.B_MIN
        random_points = np.random.rand(self.n_data*1000, 3) * length + self.B_MIN
        ...
        inside_points = inside_points[:self.n_data // 2] if nin > self.n_data // 2 else inside_points
        outside_points = outside_points[:self.n_data // 2] if nin > self.n_data // 2 else outside_points[:(self.n_data - nin)]
