...
def batch_eval(points, eval_func, num_samples=512 * 512 * 512, time_step=None, idx=None):
    ...
    for i in range(num_batches):
        sdf[i * num_samples:i * num_samples + num_samples] = eval_func(
            points[:, i * num_samples:i * num_samples + num_samples], time_step, idx)
    if num_pts % num_samples:
        sdf[num_batches * num_samples:] = eval_func(points[:, num_batches * num_samples:], time_step, idx)

    return sdf


def eval_grid(coords, eval_func, num_samples=512 * 512 * 512, time_step=None, idx=None):
    resolution = coords.shape[1:4]
    coords = coords.reshape([3, -1])
    sdf = batch_eval(coords, eval_func, num_samples=num_samples, time_step=time_step, idx=idx)
    return sdf.reshape(resolution)


def eval_grid_octree(coords, eval_func,
                     init_resolution=64, threshold=0.01,
                     num_samples=512 * 512 * 512, time_step=None, idx=None):
    resolution = coords.shape[1:4]
    ...
    while reso > 0:
        ...
        sdf[test_mask] = batch_eval(points, eval_func, num_samples=num_samples, time_step=time_step, idx=idx)
