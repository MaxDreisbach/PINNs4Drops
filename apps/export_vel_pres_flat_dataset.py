...
def extractFromSimulation(file):
    ...
    u_mapped = map_field(coord, u)
    v_mapped = map_field(coord, v)
    w_mapped = map_field(coord, w)
    p_mapped = map_field(coord, P)
    c_mapped = map_field(coord, c)
...
pathToSimulation = "/net/istmtrinculo/volume2/data2/hi208/fh2_work/foam/vu3498-3.2/run/AllesGleichWieStrukturiert/"
...
for i, file in enumerate(dirnames):
    ...
    # saving data
    savedirvel = os.path.join(out_dir, 'VEL', 'droplet' + str(i).zfill(4))
    if not os.path.exists(savedirvel):
        os.mkdir(savedirvel)

    savedirpres = os.path.join(out_dir, 'PRES', 'droplet' + str(i).zfill(4))
